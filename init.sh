...
FORCE=false
DISABLE_MEMBER_DOMAINS_POLICY=false
while [[ $# -gt 0 ]]; do
  case "$1" in
  -f | --force)
    FORCE=true
    shift
    ;;
  -d | --disable-member-domains-policy)
    DISABLE_MEMBER_DOMAINS_POLICY=true
    shift
    ;;
  *)
...
if [ "$PROMPT_AND_WRITE" = true ]; then
  ...
  # Ask for Zone (with default)
  DEFAULT_ZONE="$REGION-a"
  read -r -p "Enter your Google Cloud Region [$DEFAULT_ZONE]: " ZONE
...
if [ $DISABLE_MEMBER_DOMAINS_POLICY = true ]; then
  echo "🔌 Disabling policy member domain"
  ...
  echo "║  Remember to let the policy a few minutes to propagate before running      ║"
  echo "║  terraform                                                                 ║"
  echo "╚════════════════════════════════════════════════════════════════════════════╝"
  echo ""
fi
