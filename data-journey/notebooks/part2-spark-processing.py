...
@app.cell
def _(BQ_CATALOG_BUCKET_NAME, LOCATION, PROJECT_ID, REST_CATALOG_BUCKET_NAME):
    ...
    return bq_catalog, external_catalog, spark
...
@app.cell
def _(BQ_DATASET, bq_catalog, spark):
...
@app.cell
def _(REST_CATALOG_PREFIX, external_catalog, spark):
...
@app.cell
def _(BQ_DATASET, PROJECT_ID, spark):
...
def _(f, ridership_df):
    ...
    return (station_time_summary_df,)


@app.cell
def _(bus_lines_df, f, station_time_summary_df):
    ...
    return (bus_line_overall_times_df,)


@app.cell
def _(bus_line_overall_times_df, bus_lines_df):
...
@app.cell
def _(bus_lines_with_min_max, f, t):
...
@app.cell
def _(bus_rides, f, ridership_df):
    ...
    return (bus_rides_with_ridership,)


@app.cell
def _(bus_rides_with_ridership):
...
@app.cell
def _(bus_rides_with_ridership, pd, t):
    simulated_ride_output_schema = t.StructType(bus_rides_with_ridership.schema.fields + [t.StructField('passengers_alighting', t.LongType(), True), t.StructField('passengers_boarding', t.LongType(), True), t.StructField('remaining_capacity', t.LongType(), True), t.StructField('remaining_at_stop', t.LongType(), True), t.StructField('total_passengers', t.LongType(), True)])
    ...
    # and return an extended dataframe with all the relevant data
        ...
        for index, row in pdf.iterrows():
            if row['last_stop']:
                ...
                passengers_boarding = 0  # andle empty input DataFrame gracefully
                passengers_in_stop = 0
                current_passengers_on_bus = 0  # Return an empty DataFrame with the exact schema expected by Spark
                remaining_at_stop = 0  # This prevents UnboundLocalError when a group is empty.
            else:
                ...
                temporary_capacity = row['total_capacity'] - current_passengers_on_bus  # Sort the DataFrame by bus_stop_index to ensure correct iterative calculation
                passengers_boarding = min(temporary_capacity, passengers_in_stop)
                remaining_at_stop = passengers_in_stop - passengers_boarding
                current_passengers_on_bus = current_passengers_on_bus + passengers_boarding  # Initialize passengers on the bus at the start of this ride segment
        ...
        pdf['passengers_boarding'] = passengers_boarding_list  # for each bus stop along the ride
        pdf['remaining_capacity'] = remainig_capacity_list
        pdf['remaining_at_stop'] = remaining_at_stop_list  # If we are at the last stop of this ride, all passengers are alighting and non are boarding
        return pdf
    bus_rides_with_riders_and_totals = bus_rides_with_ridership.groupBy('bus_ride_id').applyInPandas(calculate_bus_passengers, schema=simulated_ride_output_schema)
    ...
    bus_rides_with_riders_and_totals.printSchema()  # Get passengers trying to board at this stop (passengers_in_stop), treating None as 0  # not sure they all have room  # Calculate random passengers alighting:  # It's a random normal distribution, proportional to the passengers in the station (central stations have lots of people coming on and off).  # Ensure alighting passengers do not exceed the number of passengers currently on the bus  # and also ensure it's not negative.  # update the number of passengers on the bus (this will change again soon with the number of boarding passengers)  # Calculate the number of passengers currently on the bus after alighting  # actual passengers on bus is the lower number from our remaining capacity after boarding, and people at the stop  # meaning, that we will take all the passengers waiting, if we can.  # if the capacity is lower, some will be left waiting on the bus station.  # Passengers after boarding at this stop  # Store the results for this row  # Add the new columns to the pandas DataFrame
    return (bus_rides_with_riders_and_totals,)
...
def _(
    ...
    return TABLE_NAME, bus_rides_prefix
...
def _(
    ...
    bigquery_client,
    bus_rides_prefix,
    bus_rides_with_riders_and_totals,
):
...
@app.cell
def _(BQ_CATALOG_BUCKET_NAME, bus_rides_prefix, display_blobs_with_prefix):
    display_blobs_with_prefix(BQ_CATALOG_BUCKET_NAME, bus_rides_prefix)
    return


@app.cell
def _(BQ_DATASET, PROJECT_ID, TABLE_NAME, bus_rides_with_riders_and_totals):
...
@app.cell
def _(TABLE_NAME, select_top_rows):
