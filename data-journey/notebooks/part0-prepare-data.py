    ...
    def programmatically_download_mta_data():
        ...
        while rows_got < TOTAL_NUMBER_OF_RECORDS:
            try:
                # get more data
                results = client.get(
                    "wujg-7c2s", limit=current_step, offset=rows_got
                )
...
def _():
    ...
    MTA_RAW_CSV = f"gs://{GENERAL_BUCKET_NAME}/{MTA_RAW_CSV_PATH_IN_GCS}"

    bucket = storage_client.bucket(GENERAL_BUCKET_NAME)
...
def _(MTA_RAW_CSV, dataset):
    ...
    _table_ref = dataset.table("raw_mta_data")
    ...
    _load_job = bigquery_client.load_table_from_uri(
        MTA_RAW_CSV, _table_ref, job_config=_job_config
    )
    _load_job.result()

    print("created {}.{}".format(STAGING_BQ_DATASET, _table_ref.table_id))
    return
        ...
        SELECT
            ...
            CAST(ROUND(
                ...
                CASE
                    ...
                    ELSE 0
                END
            ) AS INT64) AS ridership
        FROM `{STAGING_BQ_DATASET}.mta_data_parsed` t
        CROSS JOIN UNNEST(GENERATE_ARRAY(0, 59)) AS minute_offset;
    """
    bigquery_client.query(_query).result()
    bigquery_client.query(
...
@app.cell
def _(bus_lines, fake_stations_lst):
    bus_stations_csv = "bus_stations.csv"
    bus_lines_json = "bus_lines.json"

    with open(bus_stations_csv, "w") as _fp:
        writer = DictWriter(_fp, fieldnames=fake_stations_lst[0].keys())
        writer.writeheader()
        writer.writerows(fake_stations_lst)

    with open(bus_lines_json, "w") as _fp:
        for line in bus_lines:
            json.dump(line, _fp)
            _fp.write("\n")
    return bus_lines_json, bus_stations_csv
...
def _(bus_lines_json, dataset):
    ...
    _table_ref = dataset.table("bus_lines")
    ...
    with open(bus_lines_json, "rb") as _fp:
        _load_job = bigquery_client.load_table_from_file(
            _fp, _table_ref, job_config=_job_config
        )
        _load_job.result()
...
@app.cell
def _(bus_stations_csv, dataset):
    bus_stations_schema = [
    ...
    _table_ref = dataset.table("bus_stations")
    ...
    with open(bus_stations_csv, "rb") as fp:
        _load_job = bigquery_client.load_table_from_file(
            fp, _table_ref, job_config=_job_config
        )
        _load_job.result()
...
@app.cell
def _(bucket, bus_stations_csv):
    bus_stations_blob = bucket.blob("mta_staging_data/bus_stations.csv")
    if bus_stations_blob.exists():
        bus_stations_blob.delete()
    bus_stations_blob.upload_from_filename(bus_stations_csv)
    return


@app.cell
def _(bucket, dataset):
    bus_lines_target_glob = "mta_staging_data/bus_lines"
    bus_lines_destination_uri = "gs://{}/{}/*.parquet".format(
        GENERAL_BUCKET_NAME, bus_lines_target_glob
    )

    bus_lines_blob_list = bucket.list_blobs(prefix=bus_lines_target_glob)
    bus_lines_blob_list = [blob for blob in bus_lines_blob_list]
    bucket.delete_blobs(bus_lines_blob_list)

    bus_lines_job_config = ExtractJobConfig()
    bus_lines_job_config.destination_format = bigquery.DestinationFormat.PARQUET

    bus_lines_table_ref = dataset.table("bus_lines")
    bus_lines_extract_job = bigquery_client.extract_table(
        bus_lines_table_ref,
        bus_lines_destination_uri,
        location=LOCATION,
        job_config=bus_lines_job_config,
    )
    bus_lines_extract_job.result()
    return


@app.cell
def _(bucket, dataset):
    ridership_target_glob = "mta_staging_data/ridership"
    ridership_destination_uri = "gs://{}/{}/*.parquet".format(
        GENERAL_BUCKET_NAME, ridership_target_glob
    )

    ridership_blob_list = bucket.list_blobs(match_glob=ridership_target_glob)
    ridership_blob_list = [blob for blob in ridership_blob_list]
    bucket.delete_blobs(ridership_blob_list)

    ridership_job_config = ExtractJobConfig()
    ridership_job_config.destination_format = bigquery.DestinationFormat.PARQUET

    ridership_table_ref = dataset.table("ridership")
    ridership_extract_job = bigquery_client.extract_table(
        ridership_table_ref,
        ridership_destination_uri,
        location=LOCATION,
